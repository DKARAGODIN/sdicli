...
* *str* - текст без *space*, *equal* и *pipe*, не в кавычках
...
`cmd_name = var equal (^ space pipe)`
Если все хорошо, то str - имя переменной. Ее значением является конкатенация всех лексем справа от `equal`. Если среди этих лексем встречается `space` или `pipe`, бросается исключение. Перед склеиванием лексем, они проходят этап подстановки переменных.
